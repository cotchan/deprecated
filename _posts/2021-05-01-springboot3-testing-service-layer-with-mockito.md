...
## 2. 테스트 대상이 의존하는 객체에 @Mock

+ **Service Layer는 `Repository를 Mocking 해서 테스트해야 합니다.`**
+ **테스트 대상인 `Service Layer가 의존하는 객체는 Mock 객체로 만들어서 테스트`를 합니다.**
+ **Mock 객체로 만든다는 것의 의미는 저희가 임의로 INPUT과 OUTPUT을 정의하여 사용하겠다는 뜻입니다.**
...
+ **`실제 단위 테스트를 작성하는 부분`입니다.**
+ 위 1,2,3번은 모두 실제 테스트 함수를 작성하기 전에 필요한 전처리 과정입니다.

---

## 3-1. given - willReturn 정의

+ **`서비스 객체가 의존하는 대상의 동작을 정의하는 부분`입니다.**
+ **`@Mock 어노테이션이 붙은 객체의 동작을 정의`합니다.**
+ 예시를 들자면 서비스 객체는 Repository 객체에 의존하기 때문에 여기서 given - willReturn을 정의할 대상은 Repository 객체입니다.
